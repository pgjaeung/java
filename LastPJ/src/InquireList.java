...
public class InquireList extends JFrame implements ActionListener{

	static String[] head = new String[] {"제목","조회번호"};
	...
	public static void viewData() throws SQLException {
		if(!rs.next()){
			...
			try {
				...
				while(rs.next()) {
					model1.addRow(new Object[] {rs.getString("title"),rs.getString("idofNum")});
				}
			}catch(Exception e1) {
				e1.getStackTrace();
			}
	...
	public static void main(String[] args) throws SQLException {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InquireList frame = new InquireList();
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==answer_b) {
			...
			try {
				...
				Answer_InquirePg ap = new Answer_InquirePg();
				ap.setVisible(true);
